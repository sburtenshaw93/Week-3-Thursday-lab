...
/*
    Create a function that changes sayHelloButton's background color to black
     and its text color to white (you can use the .style object or create a CSS class and use classList.add)
...
const changeSayHelloButtonColor = () => {
...
// PROBLEM 3
/*
    Now you can see that the button colors change, but they do not change back when 
    we take the mouse off of the button.
...
// CODE HERE
 const originalSayHelloButtonColor = (color) =>{
    document.body.style.backgroundColor = color;
 }
 window.addEventListener("load", function() { originalSayHelloButtonColor('white')});
...
// Put your code in the repeatMyParam function above
...
//PROBLEM 11
/*
    You are going to add the ability to POST to the server. You'll need to create a small form and 
    write a function that makes a post request. Then you'll attach that function to the submit 
    event on the form. We'll be creating a list of foods. 

    In the index.html file inside of the client folder, create a form with one text input field 
    and a button. The input field should have a placeholder that tells the user to enter a food.
    ...
    Use a .then to handle the promise returned from the axios call. Pass a callback function 
    to the .then. Inside that callback, console log the res.data. 
...
function createFood (event) {
    ...
    let foodInpput = document.getElementById("#foodInput")
    list.innerHTML = '';
